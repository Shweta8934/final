...
# from student_utils import get_student_weak_topics
# from tutor_engine import ask_tutor_sync
# from student_db import log_interaction, get_recent_interactions, set_feedback, init_db, get_gamification, update_gamification
# from file_handler import render_file_upload_section, get_file_analysis_prompt
# from weekly_email import send_weekly_email, get_weekly_summary
...
#     subject_keywords = {
#         "Math": [
#             "solve", "equation", "algebra", "geometry", "numbers", "calculate", "formula",
#             "fraction", "integral", "derivative", "limit", "function", "graph", "polynomial",
#             "quadratic", "linear", "logarithm", "trigonometry", "sine", "cosine", "tangent",
#             "statistics", "probability", "mean", "median", "mode", "matrix", "vector", "coordinate",
#             "angle", "triangle", "circle", "rectangle", "square", "area", "perimeter", "volume",
#             "surface area", "pythagorean", "arithmetic", "multiplication", "division",
#             "addition", "subtraction", "percentage", "ratio", "proportion", "decimal", "integer",
#             "prime", "composite", "factor", "multiple", "gcd", "lcm", "inequality", "complex number",
#             "+", "-", "*", "/", "=", "<", ">", "≤", "≥", "∑", "∏", "∫", "∆"
#         ],
#         "Science": [
#             "force", "energy", "motion", "velocity", "acceleration", "gravity", "friction",
#             "momentum", "pressure", "temperature", "heat", "light", "sound", "wave",
#             "electricity", "magnetism", "current", "voltage", "resistance", "circuit",
#             "newton", "law of motion", "first law", "second law", "third law",
#             "atom", "molecule", "chemical", "reaction", "element", "compound", "mixture",
#             "acid", "base", "ph", "bond", "periodic table",
#             "cell", "organism", "dna", "rna", "gene", "photosynthesis", "ecosystem",
#             "enzyme", "protein", "chromosome", "evolution", "bacteria", "virus",
#             "earthquake", "volcano", "rock", "mineral", "fossil", "weather", "climate", "atmosphere"
#         ],
#         "English": [
#             "grammar", "syntax", "sentence", "paragraph", "noun", "verb", "adjective",
#             "adverb", "pronoun", "preposition", "conjunction", "interjection",
#             "subject", "predicate", "object", "clause", "phrase", "tense",
#             "punctuation", "comma", "period", "semicolon", "apostrophe", "quotation",
#             "essay", "write", "writing", "composition", "introduction", "conclusion",
#             "thesis", "argument", "narrative", "descriptive", "persuasive", "expository",
#             "literature", "poem", "novel", "story", "character", "plot", "theme",
#             "metaphor", "simile", "alliteration", "rhyme", "vocabulary", "synonym", "antonym"
#         ],
#         "General": []
#     }

#     math_patterns = [
#         r"\d+x", r"x[\+\-\*/]\d+", r"\d+x\^\d+", r"f\(x\)", r"[a-z]\^2",
#         r"\d+/\d+", r"√\d+", r"\d+%"
#     ]
...
#                 detected_subjects = [subject]
#             else:
#                 is_compliant, detected_subjects = check_subject_compliance(analysis_text, subject)
...
#                     other_subjects = [s for s in detected_subjects if s != subject]
#                     if other_subjects:
#                         st.warning(f"Your question is about: **{', '.join(other_subjects)}**")
#                         st.error(f"You are in **{subject}** mode. Only **{subject}** questions are allowed!")
#                     else:
#                         st.warning(f"Your question contains multiple subjects including **{', '.join(detected_subjects)}**")
#                         st.error(f"You are in **{subject}** mode. Only pure **{subject}** questions are allowed!")
#                 else:
#                     st.warning("Your question doesn't appear to be clearly about any specific subject.")
#                     st.error(f"You are in **{subject}** mode. Only **{subject}** questions are allowed!")
#             else:
#                 placeholder = st.empty()
#                 with placeholder.container():
...
#                     with placeholder.container():
#                         if extracted_text:
#                             st.success("✅ Here's the analysis of your file and answer to your question:")
#                         else:
#                             st.success("✅ Here's your answer:")
...
#                             st.markdown("### 📚 Recommended Resources")
#                             for r in resources:
#                                 st.write(f"- [{r.get('title','Resource')}]({r.get('link','#')})")

#                     inter_id = log_interaction(
#                         student_name, grade, subject, combined_question, answer_text, json.dumps(resources)
#                     )

#                     st.session_state.chat_history.insert(
#                         0,
#                         {"id": inter_id, "q": combined_question, "a": answer_text}
#                     )
...
#         for row in recent:
#             inter_id, q, a, resources, feedback, feedback_comment, created_at = row[:7]
#             with st.expander(f"🕒 {created_at} — Q: {q[:60]}{'...' if len(q) > 60 else ''}"):
#                 st.markdown("**Answer:**")
#                 st.write(a)
...
#                     )
#                     comment = st.text_input(
#                         "Optional comment", value=feedback_comment if feedback_comment else "",
#                         key=f"comment_{inter_id}"
#                     )
#                     if st.form_submit_button("Submit Feedback"):
#                         feedback_score = 1 if feedback_val == "👍 Helpful" else -1 if feedback_val == "👎 Not Helpful" else 0
#                         set_feedback(inter_id, feedback_score, comment)
#                         st.success("✅ Feedback saved!")
...
#     update_gamification(student_name, xp=0)
#     gamification = get_gamification(student_name)
#     badge_display = ", ".join(gamification['badges']) if gamification['badges'] else "No badges yet"
...
#     # ----------------- Progress charts -----------------
#     st.markdown("### 📈 Your Progress Overview")
#     df_student = pd.DataFrame(recent, columns=[
#         "id", "question", "answer", "resources", "feedback", "comment", "created_at"
#     ])

#     if not df_student.empty:
#         feedback_counts = df_student.groupby("feedback").size().reset_index(name="count")
#         feedback_counts["feedback_text"] = feedback_counts["feedback"].replace({1:"👍 Helpful", 0:"😐 Medium", -1:"👎 Not Helpful"})
#         fig_student = px.pie(feedback_counts, names="feedback_text", values="count",
#                               title="Feedback Distribution for Your Questions", hole=0.4)
#         st.plotly_chart(fig_student, use_container_width=True)

#         df_student['created_at'] = pd.to_datetime(df_student['created_at'])
#         df_student_sorted = df_student.groupby(df_student['created_at'].dt.date).size().reset_index(name='Questions')
#         fig_bar = px.bar(df_student_sorted, x='created_at', y='Questions', title="Questions Asked Over Time")
#         st.plotly_chart(fig_bar, use_container_width=True)

#     weak_topics, topic_details = get_student_weak_topics(student_name)
#     st.markdown("### ⚠️ Weak Topics & Suggested Practice")
#     if weak_topics:
#         st.warning(f"You are struggling with: {', '.join(weak_topics)}")
#         st.info("💡 Suggested: Review these topics with extra practice and resources.")
...
#         st.write(f"Viewing student: **{selected_student}** in **Grade {selected_grade}**")

#         # --- Fetch recent interactions ---
#         recent_interactions = get_recent_interactions(selected_student, selected_grade, limit=10)
...
#         if feedback_data:
#             feedback_mapping = {1: "Helpful", -1: "Not Helpful", 0: "Neutral"}
#             mapped_feedback = [(feedback_mapping.get(fb, "Unknown"), count) for fb, count in feedback_data]
...
#         weak_topics = [subj for subj, total, wrong in topics if total > 0 and (wrong / total) > 0.3]
#         st.markdown(f"### ⚠️ Weak Subjects: {', '.join(weak_topics) if weak_topics else 'None'}")
...
# st.sidebar.markdown("- Gamification placeholders (XP, badges, streaks)")
# st.sidebar.markdown("- Persistent feedback (immediate + history)")
# st.sidebar.markdown("- Multimodal input placeholders (diagram/PDF OCR) - used internally by AI")
# st.sidebar.markdown("- **🎯 Subject-specific question filtering**")


# # current streak with subject wise validation
# import os
# import streamlit as st
...
# from quiz_logic import quiz_component
# from student_utils import get_student_weak_topics
# # --- Import your existing tutor engine & student DB functions ---
# from tutor_engine import ask_tutor_sync
# from student_db import log_interaction, get_recent_interactions, set_feedback, init_db, get_gamification
# # ----------------- GAMIFICATION DISPLAY -----------------
# from student_db import update_gamification  # make sure imported
# from file_handler import render_file_upload_section, get_file_analysis_prompt

# # ----------------- Enhanced Subject Filtering Function -----------------
...
#     detected_subjects = list(set(detected_subjects))  # unique subjects

#     # General mode allows everything
#     if selected_subject == "General":
#         return True, detected_subjects

#     # If detected subject matches selected
#     if selected_subject in detected_subjects:
#         return True, detected_subjects
...
#     # NEW FILE UPLOAD SECTION - Replace your existing upload code with this:
#     extracted_text, file_info, question = render_file_upload_section(subject)

#     # Combine file content and question for AI processing
#     combined_question = ""
#     if extracted_text and question.strip():
#         combined_question = f"""I have uploaded a file with the following content:
...
#             st.warning("Please type a question or upload a file.")
#         else:
#             # For analysis, use just the question text for subject compliance
#             if extracted_text:
#                 # For file uploads, we'll be more lenient with subject checking
#                 # since files might contain mixed content
#                 analysis_text = combined_question
#             else:
#                 analysis_text = combined_question

#             # --- STRICT Subject Compliance Check (you can make this more lenient for files) ---
#             if extracted_text:
#                 # More lenient checking for file uploads
#                 is_compliant = True  # Allow file-based questions
#                 detected_subjects = [subject]  # Assume it's related to the selected subject
#             else:
#                 # Regular strict checking for text-only questions
#                 is_compliant, detected_subjects = check_subject_compliance(analysis_text, subject)
...
#             else:
#                 # Question is compliant, proceed with tutoring
#                 placeholder = st.empty()
#                 with placeholder.container():
#                     if extracted_text:
#                         st.info("🤔 Analyzing your file and question...")
...
#                     # Log the interaction
#                     inter_id = log_interaction(
#                         student_name, subject, combined_question, answer_text, json.dumps(resources)
#                     )
...
#     # Update XP and streak after each interaction
#     # (Call this after log_interaction inside the "Send" button handler)
#     update_gamification(student_name, xp=0)
...
#         # Bar chart: questions over time
#         df_student['created_at'] = pd.to_datetime(df_student['created_at'])
#         df_student_sorted = df_student.groupby(df_student['created_at'].dt.date).size().reset_index(name='Questions')
#         fig_bar = px.bar(df_student_sorted, x='created_at', y='Questions', title="Questions Asked Over Time")
#         st.plotly_chart(fig_bar, use_container_width=True)

#     # --- Weak topics + examples/resources ---
#     weak_topics, topic_details = get_student_weak_topics(student_name)
#     st.markdown("### ⚠️ Weak Topics & Suggested Practice")
#     if weak_topics:
#         st.warning(f"You are struggling with: {', '.join(weak_topics)}")
...
# else:
#     st.subheader("👨‍🏫 Teacher / Parent Dashboard")
#     st.write(f"Viewing student: **{student_name}**")
#     conn = sqlite3.connect("student.db")
#     c = conn.cursor()
...
#     topics = c.fetchall()
#     conn.close()
# # calculating weak topics based on >30% wrong feedback:
#     weak_topics = []
#     for subj, total, wrong in topics:
#         if total > 0 and (wrong / total) > 0.3:  # threshold 30%
...
#             x="subject",
#             y="count",
#             title=f"📊 {student_name}'s Learning Activity by Subject",
#             labels={"count": "Number of Questions", "subject": "Subject"},
#             color="count",
#             color_continuous_scale="Blues"
#         )
...
#     # --- Recent interactions with student-style expandable format ---
#     st.markdown("### 📋 Recent Detailed Interactions")
#     recent = get_recent_interactions(student_name,grade, limit=30)
#     if not recent:
...
#                     pass
#         # for row in recent:
#         #     inter_id = log_interaction(student_name, grade, subject, combined_question, answer_text, json.dumps(resources))
#         #     with st.expander(f"🕒 {created_at} — Q: {q[:60]}{'...' if len(q) > 60 else ''}"):
...
#                 # Display feedback (read-only for teacher/parent view)
#                 feedback_icon = "👍" if feedback == 1 else "👎" if feedback == -1 else "😐"
#                 feedback_text = "Helpful" if feedback == 1 else "Not Helpful" if feedback == -1 else "No Feedback"
#                 st.markdown(f"**{feedback_icon} Student Feedback:** {feedback_text}")

#                 if feedback_comment:
#                     st.markdown(f"**💭 Student Comment:** {feedback_comment}")

#     # Feedback pie chart
#     if feedback_data:
#         df_feedback = pd.DataFrame(feedback_data, columns=["feedback", "count"])
#         df_feedback["feedback_text"] = df_feedback["feedback"].replace({1:"👍 Helpful", 0:"😐 Medium", -1:"👎 Not Helpful"})
#         fig_feedback = px.pie(df_feedback, names="feedback_text", values="count",
#                               title="Feedback Distribution", hole=0.3)
#         st.plotly_chart(fig_feedback, use_container_width=True)

#     # --- Weak Topics Alert ---
#     st.markdown("### ⚠️ Weak Topics")
#     if weak_topics:
#         st.warning(f"The student is struggling with: {', '.join(weak_topics)}")
#         st.info("💡 Suggested: Provide extra practice in these areas.")
#     else:
#         st.success("🎉 No weak topics detected. Keep up the good work!")
...
#         "Practice a little every day and you'll master it!",
#         "Mistakes are proof you're learning — keep trying!"
#     ]
#     st.markdown("### 🌟 Motivation")
#     st.info(random.choice(motivations))

#     # --- Optional: Show examples of weak-topic questions ---
#     if weak_topics:
#         for topic in weak_topics:
#             st.markdown(f"#### 🔹 {topic} Examples of Struggle")
#             conn = sqlite3.connect("student.db")
#             c = conn.cursor()
#             c.execute("""
...
#             for q, a in examples:
#                 st.markdown(f"- ❌ Q: {q[:60]}... | A: {a[:60]}...")

# # ----------------- Sidebar Notes -----------------
# st.sidebar.markdown("---")
# st.sidebar.markdown("**ℹ️ System Information**")
# st.sidebar.success("✅ Using Enhanced AI Tutor Engine (GPT-4o-mini)")
# st.sidebar.info("💰 Optimized for adaptive learning & personalized pedagogy")
# st.sidebar.markdown("**🔧 Features:**")
# st.sidebar.markdown("- Step-by-step explanations")
# st.sidebar.markdown("- Learning hints & interactive questions")
# st.sidebar.markdown("- Curated educational resources")
